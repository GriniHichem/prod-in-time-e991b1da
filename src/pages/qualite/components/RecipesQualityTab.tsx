...
export default function RecipesQualityTab() {
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="p-3 flex items-center gap-3 flex-wrap">
          <Input placeholder="Recherche produit / recette…" value={search} onChange={(e) => setSearch(e.target.value)} className="h-12 max-w-xs" />
          ...
          {filtersActive && (
            <Button variant="ghost" size="sm" onClick={() => { setSearch(""); setStatusFilter("all"); setCcpOnly(false); }}>
                        ...
                        return (
                          <div key={r.id} className="p-4 pl-12 space-y-3">
                            ...
                            <div>
                              ...
                              {rLines.length === 0 ? <p className="text-xs text-muted-foreground">Aucun article</p> : (
                            ...
                            <div>
                              ...
                              {rSteps.length === 0 ? <p className="text-xs text-muted-foreground">Aucune étape définie</p> : (
                                  ...
                                  {rSteps.map((s) => {
                                    ...
                                    return (
                                      <div key={s.id} className="text-xs flex items-center gap-2 flex-wrap">
                                        ...
                                        {s.expected_duration_minutes && <span className="text-muted-foreground">· {s.expected_duration_minutes} min</span>}
                                        {s.critical_control_point && (
                                          <Badge variant="destructive" className="text-[9px] gap-1"><AlertTriangle className="h-3 w-3" /> CCP</Badge>
                                        )}
                                        {ind ? (
